...
import { getOrdersStatus } from '../../../services/orderServer';
...
const OrderTrackingTab = () => {
  ...
  const fetchOrdersStatus = async (status) => {
    ...
    try {
      const response = await getOrdersStatus(status, accessToken, axiosJWT);
      if (response.success) {
          ...
          <nav className="header__nav">
            {Object.entries(statusMap).map(([key, name]) => (
              <button
                ...
                onClick={() => {
                  navigate(`/info-user?tab=orders&status=${key}`);
                }}
              >
                {name}
              </button>
              ...
              {item.orderItems.map((item) => (
                <div key={item._id || item.id} className="order__item">
                  <div className="item__image">
                    <img
                      src={(item._idProduct.images && item._idProduct.images[0] && item._idProduct.images[0].url) || ''}
                      alt={item.name}
                    />
                  </div>
